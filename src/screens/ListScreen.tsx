...
export default function ListScreen({ navigation }: RootStackScreenProps<'ListScreen'>) {
	...
	const [ categoryIndex, setCategoryIndex ] = useState<number | undefined>(undefined);
	const getList = async (queryText: string) => {
		try {
			const response: IPlayerScreeenParams[] = await getPodcastList(queryText).unwrap();
			setList(response);
		} catch (error) {
			console.log('error', error);
			navigation.navigate('ErrorModalScreen', { text: 'We could not access the list now. Please try again!' });
		}
	};
	useEffect(() => {
		getList('');
